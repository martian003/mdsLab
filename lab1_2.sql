...
GROUP BY PROJECT_NUMBER
HAVING COUNT(DISTINCT PART_NUMBER) = (
...
HAVING COUNT(DISTINCT PART_NUMBER) = (
    ...
    FROM (
        SELECT PROJECT_NUMBER, COUNT(DISTINCT PART_NUMBER) AS PartCount
        FROM SHIPMENTS
        GROUP BY PROJECT_NUMBER
    ) AS ProjectPartCounts
);


SELECT PROJECT_NUMBER, 
       MAX(QUANTITY) AS SecondLargestQuantity
FROM SHIPMENTS
WHERE QUANTITY < (
    SELECT MAX(QUANTITY)
    FROM SHIPMENTS AS inner_shipments
    WHERE inner_shipments.PROJECT_NUMBER = SHIPMENTS.PROJECT_NUMBER
)
GROUP BY PROJECT_NUMBER;
...
HAVING COUNT(DISTINCT PART_NUMBER) = (
    SELECT COUNT(DISTINCT PART_NUMBER)
    FROM SHIPMENTS
    WHERE PROJECT_NUMBER IN (SELECT PROJECT_NUMBER FROM PROJECTS)
)
AND COUNT(DISTINCT PROJECT_NUMBER) = (
